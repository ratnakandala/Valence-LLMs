#!/usr/bin/env python3
# This script performs batched sentiment analysis using the GEITje-7B-ultra model,
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Fast batched, quantized valence scoring with GEITje-7B-ultra"
    )
...
def main():
    ...
    # Load tokenizer and 8-bit quantized GEITje-7B-ultra on cuda:0
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        device_map={"": device}  # Load entirely onto cuda:0
    )
    model.eval()  # Disable dropout for deterministic inference
    print(f"Loaded BramVanroy/GEITje-7B-ultra in {time.time() - start_time:.2f}s on {device}")
    ...
    df_final['geitje-7B-ultra-valences'] = df_final['valence_score']
